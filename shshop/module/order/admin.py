...
from shshop.models import (
    ShShopOrderInfo, ShShopOrderSKU
)
...
@admin.register(ShShopOrderInfo, site=sh_site)
class ShShopOrderInfoModelAdmin(BaseModelAdmin):
    ...
    def dis_order_sku(self, obj):
        order_skus = obj.shshopordersku_set.all()
    ...
    def dis_acticon_order(self, request, queryset):
        ...
        if sh_settings.HAS_MESSAGE_EAMIL:
            from django.core.mail import send_mail
            mail_addrs = queryset.values_list('owner__email', flat=True)
            mail_num = send_mail(
                subject="shShop发货通知...",
                message=f"您在shShop订购的商品已发货，请注意查收！http://www.sh.shop",
